# -*- coding: utf-8 -*-
"""
Created on Sun Jul 19 21:26:24 2026
...
UPDATED: Added uncertainty filtering for proteins with Probable/Putative/Uncharacterized annotations
"""
...
# =============================================
base_data_dir = r"D:\zebfish\zebfish_organized"
output_base_dir = r"D:\zebfish\revision\zebfish_processed_results"
...
# =============================================
# UPDATED: classify_enzymes_from_tsv with uncertainty filtering
# =============================================
def classify_enzymes_from_tsv(tsv_path, folder_name):
    """
    Classify proteins as enzymes based on EC numbers from TSV file
    
    UPDATED: Filters out proteins with uncertain annotations
    (Probable, Putative, Uncharacterized, Unknown, Predicted, Possible, Likely)
    """
    ...
    try:
        ...
        # =============================================
        # NEW: FILTER UNCERTAIN ANNOTATIONS
        # =============================================
        uncertain_terms = ['probable', 'putative', 'uncharacterized', 
                          'unknown', 'predicted', 'possible', 'likely']
        
        # Check if Protein names column exists
        if 'Protein names' in df.columns:
            # Create mask for uncertain annotations
            uncertain_mask = pd.Series(False, index=df.index)
            for term in uncertain_terms:
                uncertain_mask |= df['Protein names'].str.contains(term, case=False, na=False)
            
            uncertain_count = uncertain_mask.sum()
            print(f"    Found {uncertain_count} proteins with uncertain annotations (Probable, Putative, etc.)")
            
            # Store uncertain proteins for tracking (before removal)
            uncertain_proteins = df[uncertain_mask]['Entry'].tolist() if 'Entry' in df.columns else []
            
            # Remove uncertain proteins
            df = df[~uncertain_mask].copy()
            print(f"    After filtering: {len(df)} proteins remaining")
            
            # Store uncertainty info for statistics
            uncertain_proteins_count = uncertain_count
        else:
            print(f"    WARNING: 'Protein names' column not found, skipping uncertainty filtering")
            uncertain_proteins_count = 0
            uncertain_proteins = []
        ...
        # NEW: Track uncertain annotation status
        df['Uncertain_Annotation'] = False  # All remaining proteins are high-confidence
        ...
        return df, enzyme_count, non_enzyme_count, uncertain_proteins_count, uncertain_proteins
...
def merge_embeddings_with_labels(embeddings_df, enzyme_df, folder_name):
    ...
    try:
        ...
        # NEW: Mapping for uncertainty
        id_to_uncertain = dict(zip(enzyme_df[enzyme_id_col], enzyme_df['Uncertain_Annotation']))
        ...
        embeddings_df['Uncertain_Annotation'] = embeddings_df[embeddings_id_col].map(id_to_uncertain)
        ...
        embeddings_df['Uncertain_Annotation'] = embeddings_df['Uncertain_Annotation'].fillna(False)
...
def main():
    ...
    print("UPDATED: Filtering uncertain annotations (Probable, Putative, etc.)")
    print("=" * 70)
    ...
    all_combined_data = []
    uncertainty_tracking = []  # NEW: Track uncertain proteins per species
    ...
    for folder_name in fish_folders:
        ...
        # Step 3: Classify enzymes from TSV (UPDATED: with uncertainty filtering)
        enzyme_df, enzyme_count, non_enzyme_count, uncertain_count, uncertain_proteins = classify_enzymes_from_tsv(tsv_path, folder_name)
        if enzyme_df is None:
            continue
        
        # Track uncertainty
        uncertainty_tracking.append({
            'Species': folder_name,
            'Uncertain_Proteins_Removed': uncertain_count,
            'Uncertain_Protein_IDs': uncertain_proteins
        })
        ...
        if merged_df is not None:
            ...
            statistics['TSV_Genes'].append(len(enzyme_df) + uncertain_count)  # Original TSV count
            statistics['Uncertain_Removed'].append(uncertain_count)  # NEW
            ...
            print(f"     - Uncertain annotations removed: {uncertain_count}")  # NEW
    ...
    if all_combined_data:
        ...
        # Save uncertainty tracking
        uncertainty_df = pd.DataFrame(uncertainty_tracking)
        uncertainty_path = os.path.join(stats_dir, "uncertain_annotations_removed.csv")
        uncertainty_df.to_csv(uncertainty_path, index=False)
        print(f"✅ Uncertainty tracking saved to: {uncertainty_path}")
        ...
        print("\n📊 Uncertainty Filtering Summary:")
        print("-" * 50)
        total_uncertain = sum(statistics['Uncertain_Removed'])
        total_tsv = sum(statistics['TSV_Genes'])
        print(f"  Total uncertain proteins removed: {total_uncertain}")
        print(f"  Total TSV entries after filtering: {total_tsv - total_uncertain}")
        print(f"  Percentage removed: {total_uncertain/total_tsv*100:.1f}%")
        
        for i, folder in enumerate(statistics['Folder']):
            if statistics['Uncertain_Removed'][i] > 0:
                print(f"    {folder:15s}: {statistics['Uncertain_Removed'][i]:>4} removed")
        
        print("\n📊 Per-Species Processing Summary:")
        print("-" * 50)
        for i, folder in enumerate(statistics['Folder']):
            print(f"\n📁 {folder.upper()}:")
            print(f"   HDF5 proteins:     {statistics['HDF5_Genes'][i]:>6}")
            print(f"   TSV entries:       {statistics['TSV_Genes'][i]:>6}")
            print(f"   Uncertain removed: {statistics['Uncertain_Removed'][i]:>6}")  # NEW
        ...
        print(f"  Total uncertain removed:    {sum(statistics['Uncertain_Removed']):>6}")  # NEW
        ...
        # NEW: Save a summary of uncertainty filtering
        uncertainty_summary = {
            'Total_Proteins_Examined': sum(statistics['TSV_Genes']),
            'Uncertain_Removed_Total': total_uncertain,
            'Uncertain_Percentage': f"{total_uncertain/sum(statistics['TSV_Genes'])*100:.1f}%",
            'Final_Dataset_Size': len(labeled_only),
            'Enzymes_Remaining': len(enzymes_only),
            'Non_Enzymes_Remaining': len(labeled_only) - len(enzymes_only)
        }
        uncertainty_summary_df = pd.DataFrame([uncertainty_summary])
        uncertainty_summary_path = os.path.join(stats_dir, "uncertainty_filtering_summary.csv")
        uncertainty_summary_df.to_csv(uncertainty_summary_path, index=False)
        print(f"✅ Uncertainty summary saved to: {uncertainty_summary_path}")
