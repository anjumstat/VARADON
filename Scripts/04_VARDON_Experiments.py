...
Models:
- 3 baselines:
  1) Softmax Logistic Regression / Linear Softmax Classifier
  2) Gaussian Dropout Neural Network
  3) Standard Bernoulli Dropout Neural Network
- 5 proposed/modified VARDON variants:
  1) VARDON_RealVD
  2) VARDON_AdaptiveVD
  3) VARDON_RealVD_Adaptive
  4) VARDON_Full
  5) VARDON_Light
"""
...
# Comment these two lines if you want to use GPU.
try:
...
BASE_DIR = r"D:\zebfish\VARDON\VARDON_Results_Corrected_0.01"
os.makedirs(BASE_DIR, exist_ok=True)

# You can expand this later, for example [0.01, 0.001, 0.0001]
...
# Choose the metric used to select the final model before test evaluation.
...
class RealVariationalDropout(layers.Layer):
    ...
    Note: This is a custom dropout approximation. Avoid overclaiming full Bayesian
    variational inference unless KL terms/objective are formally added.
    """
...
class AdaptiveVariationalDropout(layers.Layer):
    ...
    def call(self, inputs, training=None):
        ...
        # Correct inference behavior for inverted dropout.
        if not training:
            return inputs
...
class FeatureImportanceGate(layers.Layer):
    ...
    def call(self, inputs):
        ...
        # Encourage the average gate activation to stay near keep_ratio.
...
def build_dropout_model(input_shape, num_classes, learning_rate, variant="real_vd_only"):
    ...
    if variant in ["real_vd_only", "real_adaptive", "full", "light"]:
        x = RealVariationalDropout(512, init_drop_rate=0.2, name="rvd1")(x)
    if variant in ["adaptive_vd_only", "real_adaptive", "full", "light"]:
...
def build_model(method_name, config, input_shape, num_classes, learning_rate):
    ...
    if config["type"] == "dropout_variant":
...
def run_cross_validation_experiment(
    ...
    summary = {
        ...
        # Keep fold values for real statistical testing.
...
def run_final_test_evaluation(best_result, X_train_val_raw, y_train_val, X_test_raw, y_test, test_indices):
    ...
    # Correct final scaling: fit only on full training+validation data.
    ...
    # Save final independent test result as CSV.
    ...
    # Also save one combined CSV containing the selected model's CV summary
    # together with its independent test performance. This is convenient for
    # manuscript tables.
...
def run_all_models_test_evaluations(results_list, X_train_val_raw, y_train_val, X_test_raw, y_test, test_indices):
    ...
    Important:
    These test results are saved for comparison/reference only. Model selection
    should still be based on cross-validation results, not test-set performance.
    """
    ...
    for r in successful:
        ...
        # Fit scaler only on full training+validation data, then transform test data.
        final_scaler = StandardScaler()
        X_train_val = final_scaler.fit_transform(X_train_val_raw).astype(np.float32)
        X_test = final_scaler.transform(X_test_raw).astype(np.float32)
    ...
    all_test_df.to_csv(os.path.join(BASE_DIR, "Paper1_Test_Results_All_Models.csv"), index=False)

    print()
    print(f"All-model independent test results saved to: {os.path.join(BASE_DIR, 'Paper1_Test_Results_All_Models.csv')}")
    return all_test_df
...
def run_statistical_analysis(results_list, output_dir, metric="fold_mcc_values"):
    ...
    Note: Folds are reused across settings with the same split seed, so paired tests
    are reasonable as a first-pass comparison. For journal submission, report and
    interpret these tests carefully.
    """
    ...
    # Best model according to mean of chosen fold metric.
    ...
    # Holm correction for pairwise p-values.
...
if __name__ == "__main__":
    ...
    print("For Enzyme vs Non-enzyme Gene Classification")
    print("=" * 80)
    print(f"Data path: {DATA_PATH}")
    print(f"Results directory: {BASE_DIR}")
    print(f"Models: {len(METHODS)} total = 5 proposed/modified + 3 baselines")
    ...
    print("The test set will be used only once after CV-based model selection.")
    ...
    # 3) Real statistical analysis on CV fold values.
